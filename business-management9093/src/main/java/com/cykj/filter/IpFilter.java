 ...
 * @version 1.0
 * @author: yitong
 * @date: 2021/8/13 22:54
 * @desc:
 */
@Slf4j
@WebFilter(urlPatterns="/traffic/*")
public class IpFilter implements Filter {
	/**
	 * 默认限制时间（单位：ms）,
	 */
	private static final long LIMITED_TIME_MILLIS = 24 * 60 * 60 * 1000;
	 ...
	 */
	private static final int MIN_SAFE_TIME = 10000;
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain)
		...
		// 得到新的key值：访问信息
		String vid = ip + '-' + type + '-' + id;
		System.out.println("当前访问信息为" + vid);
		// 获取IP存储器：存储访问过的IP信息
		ConcurrentHashMap<String, Long> ipMap = (ConcurrentHashMap<String, Long>) context.getAttribute("ipMap");
		// 过滤IP
		filterIpMap(ipMap);
		// 判断存储器中是否存在当前访问信息，如果没有则为初次访问，初始化该ip
		if(!ipMap.containsKey(vid)) {
			ipMap.put(vid, System.currentTimeMillis());
			// 放行
...
//		context.setAttribute("ipMap", ipMap);
	 ...
	 * @param ipMap
	 */
	private void filterIpMap(ConcurrentHashMap<String, Long> ipMap) {
		if(ipMap == null) {
			return;
		}
		...
		while(keyIt.hasNext()) {
			Long visitTime = ipMap.get(keyIt.next());
			if(visitTime + LIMITED_TIME_MILLIS <= System.currentTimeMillis()){
				keyIt.remove();
			}
		}
	}
