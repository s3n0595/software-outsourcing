...
//            String realPath = ResourceUtils.getURL("classpath:").getPath() + "EmpFolder";
            ...
            String uuid = UUID.randomUUID().toString();
            fn = uuid + '_' + file.getOriginalFilename();
            ;//uuid+文件名
            File saveFile = new File(realPath, fn);
            file.transferTo(saveFile);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    //    小程序端发布需求
    @PostMapping("wechat/demand")
    public String wechatDemand(Demand demand) {
        System.out.println(demand);
        Date date = new Date();
        demand.setAnnexPath(fn);
