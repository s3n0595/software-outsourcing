  ...
  <div>
    ...
    <div class="section-21WOI clearfix" style="margin-left: 10%">
      ...
      <div style="margin-top: 3%">
        <span>名&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;称:</span>
        <span v-if="nameShow" style="margin-left: 100px;">{{infoList.providerName}}</span>
...
export default {
  ...
  methods: {
      ...
      this.$refs['ruleForm'].validate((valid)=>{
        if(valid){
          if(this.ruleForm.code===sessionStorage.getItem("checkCode")){
            this.$axios.post('empcenter/editproEmail',this.$qs.stringify({
    ...
    nameconfirm(){
      this.$axios.post('empcenter/editProName',this.$qs.stringify({
        providerId:this.user.providerId,
        providerName:this.oldName
      })).then(res=>{
        if(res.data==1){
          this.$notify.success({
            title: '名称修改成功',
    ...
    seleInfo(){
      this.$axios.post('empcenter/procenterInfo',this.$qs.stringify({
        providerId:this.user.providerId
      })).then(res=>{
        this.infoList=res.data;
        if(res.data.headPath!='' && res.data.headPath!=null){
  ...
  mounted() {
    ...
    this.userForm.Id=this.user.providerId;
